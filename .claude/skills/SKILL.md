...
description: 기능 개발/버그 수정 시 테스트 작성 원칙 참고용 (Python + unittest). 이 프로젝트에서는 아래 "프로젝트 오버라이드" 적용.
---

# 테스트 주도 개발 (TDD) — Python / unittest

## 프로젝트 오버라이드 (이 프로젝트에서는 이 섹션이 우선)

SampleOrderSystem에서는 사이클당 속도를 위해 아래 두 가지를 **적용하지 않는다**:
- RED 단계(테스트를 코드보다 먼저 작성하고, 실패하는 것을 직접 확인하는 절차)
- 테스트를 코드보다 먼저 작성하지 않았다고 해서 코드를 삭제하고 처음부터 다시 작성하는 것

대신 다음을 따른다:
1. `Plan.md`와 관련 `docs/*.md`를 먼저 갱신한다(요구사항/설계 정리).
2. 코드를 구현한다.
3. 테스트는 구현과 동시에 또는 직후에 작성한다. 새/변경된 기능은 정상 케이스 + 실패 케이스를 최소 1개씩 포함하고, `python -m unittest discover tests`가 전부 통과하면 된다. 테스트가 실패하는 순간을 직접 봐야 한다는 요구는 없다.

아래 "개요"부터의 본문은 TDD의 일반 원칙(왜 테스트가 중요한지, 좋은 테스트란 무엇인지)에 대한 배경 지식으로 참고하되, RED 우선 절차와 "코드를 먼저 쓰면 삭제하라"는 지시는 이 프로젝트에서 실행하지 않는다.
...
> 위 두 문단은 TDD 일반론이며, 이 프로젝트에서는 위 "프로젝트 오버라이드"가 우선한다.
...
## 절대 법칙 (일반 TDD 원칙 — 이 프로젝트에서는 "프로젝트 오버라이드" 참고)
...
> SampleOrderSystem에서는 이 절을 적용하지 않는다. 코드를 먼저 쓰고 테스트를 나중에 붙여도 삭제하지 않는다. 위 "프로젝트 오버라이드" 참고.
...
## 위험 신호 — 일반 TDD 기준(이 프로젝트에는 적용하지 않음)

아래 목록은 일반적인 TDD 관점의 위험 신호이며, SampleOrderSystem에서는 "테스트보다 먼저 작성된 코드"/"구현 후 작성된 테스트"를 문제로 보지 않는다(프로젝트 오버라이드 참고). 나머지(테스트를 아예 안 쓰기, 새 기능에 실패 케이스 없이 넘어가기 등)는 이 프로젝트에서도 여전히 피해야 한다.

- ~~테스트보다 먼저 작성된 코드~~ (이 프로젝트에서는 허용)
- ~~구현 후 작성된 테스트~~ (이 프로젝트에서는 기본 방식)
- 테스트가 즉시 통과(잘못된 것을 테스트하고 있을 가능성 — 테스트 자체를 검토)
- 테스트가 왜 필요한지/무엇을 검증하는지 설명할 수 없음
- 새/변경된 기능에 정상 케이스 또는 실패 케이스가 하나도 없음
- 전체 테스트를 돌려보지 않고 "될 것 같다"며 넘어감

**이 프로젝트에서의 의미: 위 항목이 걸리면 테스트를 보강하거나 다시 확인하되, 코드를 삭제하고 처음부터 다시 쓸 필요는 없다.**
