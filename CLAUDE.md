...
- 비즈니스 규칙(상태 전이, 재고 검증)은 `services.py`에만 둔다. `cli.py`와 `repository.py`에는 넣지 않는다.
- 잘못된 입력/상태는 `ValueError` 또는 `KeyError`를 던진다. `cli.py`의 최상위 루프에서만 캐치해서 사용자에게 메시지로 보여준다.
